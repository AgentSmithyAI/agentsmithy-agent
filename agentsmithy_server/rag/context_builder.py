...
class ContextBuilder:
    ...
    async def build_context(
        ...
        if file_context and file_context.get("open_files"):
            for i, file_info in enumerate(file_context["open_files"][:settings.max_open_files]):
        ...
        if query:
            ...
            for doc in relevant_docs:
                remaining_space = self.max_context_length - context["total_context_length"]
    ...
    def format_context_for_prompt(self, context: Dict[str, Any]) -> str:
        ...
        if context.get("current_file"):
            ...
            formatted_parts.append(f"=== Current File: {cf['path']} ({cf['language']}) ===")
            if cf.get("selection"):
