...
class UvicornJSONFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if hasattr(record, "scope"):
            ...
            log_data["client"] = f"{scope.get('client', ['', ''])[0]}:{scope.get('client', ['', ''])[1]}"
...
class UvicornPrettyFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        # Time in gray
        time_str = f"{self.GRAY}{datetime.utcnow().strftime('%H:%M:%S.%f')[:-3]}{self.RESET}"
...
formatter_class = UvicornPrettyFormatter if os.getenv("LOG_FORMAT", "pretty").lower() == "pretty" else UvicornJSONFormatter
formatter_name = "pretty" if os.getenv("LOG_FORMAT", "pretty").lower() == "pretty" else "json"
