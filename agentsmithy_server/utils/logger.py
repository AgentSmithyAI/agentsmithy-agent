...
class PrettyFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if extras:
            parts.append(f"{self.GRAY}|{self.RESET} " + f" {self.GRAY}|{self.RESET} ".join(extras))
...
class StructuredLogger:
    ...
    def error(self, message: str, exception: Optional[Exception] = None, **kwargs):
        ...
        if exception:
            ...
            self.logger.error(message, exc_info=exception, extra={"extra_fields": kwargs})
        else:
            self._log(logging.ERROR, message, **kwargs)
    
    def request_log(self, method: str, path: str, status_code: int, duration_ms: float, **kwargs):
