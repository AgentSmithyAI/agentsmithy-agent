...
def test_update_config(client: TestClient):
    ...
    assert "Successfully updated 2 configuration key(s)" in data["message"]

    # Check updated config
    config = data["config"]
    assert config["model"] == "gpt-4-turbo"
    assert config["temperature"] == 0.9
    assert config["max_tokens"] == 2000  # Unchanged
...
def test_update_config_persists(client: TestClient, config_file: Path):
    ...
    assert saved_config["model"] == "gpt-3.5-turbo"
...
def test_get_config_after_update(client: TestClient):
    ...
    initial_model = response1.json()["config"]["model"]
    assert initial_model == "gpt-4"

    # Update config
    update_data = {"config": {"model": "claude-3-sonnet"}}
    ...
    updated_model = response3.json()["config"]["model"]
